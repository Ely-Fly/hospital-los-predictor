...
# train = pd.read_csv('/Users/ellayuan/Desktop/Cursor/lesson3/titanic/test.csv')

# print(train.head())

# # write a loop that counts how many files in a folder end with .csv or .xlsx.

# from pathlib import Path 
# folder_path = Path('/Users/ellayuan/Desktop/Cursor/lesson3/titanic')
# ini_cnt = 0
# for file in folder_path.iterdir():
#     if file.suffix in ['.cvs', 'xlsx']:
#         ini_cnt += 1

# print(f"There are {ini_cnt} files in the folder.")

# cnt = len([f for f in folder_path.iterdir() if f.suffix in ['csv','.xlsx']])
# print(cnt) 

# file_names = [f.name for f in folder_path.iterdir() if f.suffix in ['.csv','.xlsx']]
# print(file_names)

# # create a dictionary comprehension to count how many times each word appears in a string 

# test = {'a': 'qwdxscerfbbhhhhhhbxbbbbbbbhhhhjjjmmmmbhbh',
#         'b': 'dededefefvcdcdewecfsdcdfvrvrvswc'}

# for key,value in test.items():
#     word_count = {char: value.count(char) for char in set(value)}
#     print(f"In key '{key}', the word count is: {word_count}")
# from collections import Counter
# counter = Counter(test['a'])
# #counter = {key: Counter(value) for key, value in test.items()}
# print(counter)

# # Write a function that automatically cleans column names (e.g., lowercases, replaces spaces with _)
# def clean_column_names(train):
#     train.columns = [col.lower().replace(' ','_') for col in train.columns]
# clean_column_names(train)
# print(train.columns)

# print(train.info())

# def clean_and_save(df,pclass):
#     df = df[df['pclass'] == pclass]
#     df.to_csv('titanic_2025', index =False)
#     print(f"File titanic_2025_{pclass}.csv saved.")
# clean_and_save(train,3)


# ######################## module 1 practice set ##########################
# ######## File handling & loops ########
# from pathlib import Path 
# folder_path = Path('/Users/ellayuan/Desktop/Cursor/lesson3/pandas/Complete')

# files = [f for f in folder_path.rglob('*') if f.suffix in ['.csv','.xlsx']]
# print(f"found {len(files)} files with .csv or .xlsx suffix.")
...
# # clean column names and save each as *_cleaned.csv 
# for f in folder_path.glob('*.csv'):
#     df = pd.read_csb(f)
#     df.columns = [col.lower().replace(' ','_') for col in df.columns]
#     new_name = f.stem + '_cleaned.csv'
#     df.to_csv(folder_path/new_name,index= False)
#     print(f"File {new_name} saved.")

# #summarized each csv files 
# # file name, rows, cols, missing
# summary = []
# for f in folder_path.glob('*.csv'):
#     df = pd.read_csv(df)
#     summary.append({
